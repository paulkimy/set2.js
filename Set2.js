...
function shiftLetter(letter, shift) {
    return letter === ' ' ? ' ' : String.fromCharCode((letter.charCodeAt(0) - 65 + shift) % 26 + 65);
}
...
function caesarCipher(message, shift) {
    function shiftLetter(letter, shift) {
        ...
        const shiftedCode = ((letterCode - 'A'.charCodeAt(0) + shift) % alphabetSize) + 'A'.charCodeAt(0);
...
function shiftByLetter(letter, letterShift) {
    ...
    const newLetterCode = ((letter.charCodeAt(0) - 'A'.charCodeAt(0) + shift) % 26) + 'A'.charCodeAt(0);
...
function scytaleCipher(message, shift) {
    ...
    let row = length / shift
    
    for (let i = 0; i < length; i++) {
        result += message[Math.floor(i / shift) + (row) * (i % shift)];
    }
    return result;
}

console.log(scytaleCipher('INFORMATION_AGE', 4));
...
function scytaleDecipher(message, shift) {
    ...
    const rows = length / shift;

    let result = '';
    
    for (let i = 0; i < length; i++) {
        result += message[Math.floor(i % rows) * shift + Math.floor(i / rows)];
    }
    return result;
}

console.log(scytaleDecipher('IRIANMOGFANEOT__', 4));
console.log(scytaleDecipher('IMNNA_FTAOIGROE', 3));
console.log(scytaleDecipher('AOTSRIOALRH_EMRNGIMA_PTT', 8));
